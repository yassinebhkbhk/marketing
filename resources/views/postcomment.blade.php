        ...
        @foreach ($comments as $comment)
          <div class="comment-container">
            <div class="border-b border-gray-200 pb-4">
              <h2 class="text-lg font-semibold mb-2">{{ $loop->iteration }} - {{ $comment->name }} {{ $comment->created_at->format('D, d M Y H:i:s') }} ({{ $comment->created_at->diffForHumans() }})</h2>
              <p class="comment-text">{{ $comment->message }}</p>
              @if (count($comment->analyseCommentaires))
                <p class="text-gray-500">Likes: {{ $comment->analyseCommentaires[0]['like_count'] }}, Comments: {{ $comment->analyseCommentaires[0]['comment_count'] }}</p>
              @endif
            </div>
          </div>
        @endforeach
